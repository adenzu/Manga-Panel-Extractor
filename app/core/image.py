...
def adaptive_vconcat(
    images: list[np.ndarray], fill_color: tuple[int, int, int] = (255, 255, 255)
) -> np.ndarray:
...
def group_contours_vertically(contours) -> list[list[np.ndarray]]:
    ...
    for contour in contours[1:]:
        ...
        for group in grouped_contours[::-1]:
            ...
            if abs(group_x_center - contour_x_center) < ERROR_THRESHOLD * min(
                group_w, contour_w
            ):
...
def group_contours_horizontally(contours) -> list[list[np.ndarray]]:
    ...
    for contour in contours[1:]:
        ...
        for group in grouped_contours[::-1]:
            ...
            if abs(group_y_center - contour_y_center) < ERROR_THRESHOLD * min(
                group_h, contour_h
            ):
                group.append(contour)
                found_group = True
...
def group_bounding_boxes_vertically(
    bounding_boxes,
...
def group_bounding_boxes_horizontally(
    bounding_boxes,
) -> list[list[tuple[int, int, int, int]]]:
    """
