...
from app.core.image import (
    ...
    group_contours_horizontally,
    group_contours_vertically,
    adaptive_hconcat,
    adaptive_vconcat,
    group_bounding_boxes_horizontally,
    group_bounding_boxes_vertically,
)
...
def get_background_intensity_range(
    grayscale_image: np.ndarray, min_range: int = 1
) -> tuple[int, int]:
    """
...
def generate_background_mask(grayscale_image: np.ndarray) -> np.ndarray:
    ...
    whole_background_min_height = mask_height * (
        1 - base_background_size_error_threshold
    )
    ...
    for i in np.argsort(stats[1:, 4])[::-1]:
        ...
        if area < halting_area_size:
            break
        if (
            (w > whole_background_min_width)
            or (h > whole_background_min_height)
            or (
                is_contour_rectangular(
                    cv2.findContours(
...
def extract_panels(
    ...
    for contour in panel_contours:
        ...
        if mode == "masked":
            ...
            fitted_panel = cv2.bitwise_or(
                cv2.bitwise_and(
                    cv2.bitwise_not(mask[y : y + h, x : x + w]), fill_in_color
                ),
                fitted_panel,
            )
        else:
...
def joint_panel_split_extraction(
    grayscale_image: np.ndarray, background_mask: np.ndarray
) -> np.ndarray:
    """
    ...
    down_right_dilation_kernel = np.pad(
        down_right_dilation_kernel, ((0, min_based_size // 2), (0, min_based_size // 2))
    )

    up_left_dilation_kernel = np.identity(min_based_size // 2 + 1, dtype=np.uint8)
    up_left_dilation_kernel = np.pad(
        up_left_dilation_kernel, ((min_based_size // 2, 0), (0, min_based_size // 2))
    )

    up_right_dilation_kernel = np.flip(
        np.identity(min_based_size // 2 + 1, dtype=np.uint8), axis=1
    )
    up_right_dilation_kernel = np.pad(
        up_right_dilation_kernel, ((min_based_size // 2, 0), (0, min_based_size // 2))
    )

    down_left_dilation_kernel = np.flip(
        np.identity(min_based_size // 2 + 1, dtype=np.uint8), axis=1
    )
    down_left_dilation_kernel = np.pad(
        down_left_dilation_kernel, ((0, min_based_size // 2), (min_based_size // 2, 0))
    )
    ...
    def get_dots(
        grayscale_image: np.ndarray, kernel: np.ndarray
        ...
        temp = cv2.copyMakeBorder(
            temp,
            pad_height,
            kernel.shape[0] - pad_height - 1,
            pad_width,
            kernel.shape[1] - pad_width - 1,
            cv2.BORDER_CONSTANT,
            value=0,
        )
        return temp
    ...
    background_mask = cv2.dilate(
        background_mask, np.ones((dilation_size, dilation_size), np.uint8), iterations=1
    )
...
def is_contour_sufficiently_big(
    contour: np.ndarray, image_height: int, image_width: int
) -> bool:
...
def threshold_extraction(
    ...
    processed_image = cv2.dilate(
        processed_image, np.ones((3, 3), np.uint8), iterations=2
    )
    ...
    contours = list(
        filter(
            lambda c: is_contour_sufficiently_big(c, image.shape[0], image.shape[1]),
            contours,
        )
    )
...
def get_page_without_background(
    grayscale_image: np.ndarray, background_mask: np.ndarray, split_joint_panels=False
) -> np.ndarray:
    """
    ...
    if STRIPE_FORMAT_MASK_AREA_RATIO > mask_area_ratio and split_joint_panels:
        page_without_background = joint_panel_split_extraction(
            grayscale_image, background_mask
        )
    else:
...
def generate_panel_blocks(
    ...
    page_without_background = get_page_without_background(
        grayscale_image, background_mask, split_joint_panels
    )
    contours, _ = cv2.findContours(
    ...
    def get_panels(contours):
        ...
        panels = get_fallback_panels(
            image, grayscale_image, fallback, panels, mode=mode
        )
        return panels
...
def generate_panel_blocks_by_ai(
    image: np.ndarray, merge: str = MergeMode.NONE
) -> list[np.ndarray]:
    """
    ...
    )  # Ignore 'FutureWarning: `torch.cuda.amp.autocast(args...)` is deprecated. Please use `torch.amp.autocast('cuda', args...)` instead.'
    ...
    for detection in results.xyxy[
        0
    ]:  # Access predictions in (x1, y1, x2, y2, confidence, class) format
